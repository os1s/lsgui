...
class Window(Frame):
    ...
    def init_window(self):
        ...
        self.master.title('paska')
        self.pack(fill=BOTH, expand=1)

        butt = Button(self, text='alkaizerX', width=12, height=2, command=self.alkaizerx)
        butt.place(x=150, y=0)

        butt = Button(self, text='chat', width=12, height=2, command=self.alkaizerx(chat=True))
...
#def streamer(self, name):
#    os.system('C:\\Users\\scrsm\\AppData\\Local\\Programs\\Python\\Python37-32\\Scripts\\livestreamer.exe twitch.tv/'+name+' best')
